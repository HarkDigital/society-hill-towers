...
STAGES = [
    ...
    S('pack_wide', 'pack_wide.py',
      inputs=['scene_wide.json', 'scene_south.json?', 'parts_wide.json?', 'wide_landmarks_research.json?', 'lidar_cache/roof_shapes.json?', 'lidar_city_roofs.json?', 'wall_palette.json?', 'wall_colors.json?'],
      outputs=['wide.b64', 'wide_walls.b64'], needs=['process_wide', 'process_south', 'lidar_join', 'patch_scenes_facade', 'roof_tags', 'fetch_lidar_roofs', 'bake_wall_colors'],
      note='parts_wide.json (building:part pieces) is hand-maintained'),
    S('pack_city', 'pack_city.py',
      inputs=['osm_city_raw.json', 'lidar_city_heights.json?', 'lidar_cache/opa_city.json?', 'lidar_cache/roof_city.json?', 'lidar_cache/roof_shapes.json?', 'lidar_city_roofs.json?'],
      outputs=['city.b64'], needs=['fetch_city', 'lidar_join', 'opa_join', 'roof_colors', 'roof_tags', 'fetch_lidar_roofs'],
    ...
    S('fetch_parking', 'fetch_parking.py',
      outputs=['parking_south.json'], fetch=True,
      note='the sports complex\'s surface lots (amenity=parking) as asphalt flats; raw answer cached in lidar_cache/'),
    S('fetch_paved', 'fetch_paved.py',
      outputs=['osm_paved_raw.json', 'paved_tiles/*.json'], fetch=True,
      note='surface parking, rail yards, ports, depots, aprons and works over the far-ring box, 16 tiles, ways only (overpass-api.de only: the other mirrors time out)'),
    S('pack_paved', 'pack_paved.py',
      inputs=['osm_paved_raw.json', 'osm_landuse_raw.json'], outputs=['paved.b64'], needs=['fetch_paved', 'fetch_landuse'],
      note='shapely; the paved ground (lots, industrial / port / retail yards, rail yards, aprons) as CCW whole-metre rings, one union per kind'),
    S('roof_tags', 'roof_tags.py',
      inputs=['osm_wide_raw.json', 'osm_south_raw.json', 'osm_city_raw.json'], outputs=['lidar_cache/roof_shapes.json'],
      needs=['fetch_wide', 'fetch_south', 'fetch_city'], note='OSM roof:shape tags and wide/south way centroids for the packers'),
    S('fetch_lidar_roofs', 'fetch_lidar_roofs.py',
      outputs=['lidar_city_roofs.json'], fetch=True,
      note='citywide roof forms streamed from the NOAA COPC tiles at coarse resolution; hours, resumable per tile'),
    S('fetch_overture', 'fetch_overture.py',
      outputs=['lidar_cache/overture_phl.json'], fetch=True, note='Overture buildings over the box via DuckDB; a cross-check, its roof and colour tags are OSM verbatim'),
    S('fetch_shops', 'fetch_shops.py',
      outputs=['lidar_cache/shops_raw.json', 'lidar_cache/shop_tiles/*.json'], fetch=True,
      note='OSM businesses over the wide and south boxes'),
    S('bake_storefronts', 'bake_storefronts.py',
      inputs=['lidar_cache/shops_raw.json', 'scene.json', 'scene_wide.json', 'scene_south.json'], outputs=['storefronts.b64'],
      needs=['fetch_shops', 'process_core', 'process_wide', 'process_south'], note='shapely; one storefront per business on its street-facing wall'),
    S('bake_schuylkill', 'bake_schuylkill.py', inputs=[], outputs=['schuylkill.json'], fetch=True,
      note='the Schuylkill\'s real course from OSM waterway ways: the carve follows it and the park reach gets its water sheet'),
    S('bake_rail', 'bake_rail.py', inputs=[], outputs=['rail_amtrak.json'], fetch=True,
      note="Amtrak's tracks through the city from OSM railway=rail ways operated by Amtrak (Overpass, cached): the live trains snap to them and the corridor is drawn (Round 80)"),
    S('fetch_patco', 'fetch_patco.py', inputs=[], outputs=['lidar_cache/patco_raw/PATCO_GTFS.zip', 'lidar_cache/patco_raw/osm_patco.json'], fetch=True,
      note="PATCO's public GTFS timetable and its OSM track between 8th and Market and Broadway (Round 141)"),
    S('bake_patco', 'bake_patco.py', inputs=['lidar_cache/patco_raw/PATCO_GTFS.zip', 'lidar_cache/patco_raw/osm_patco.json'], outputs=['patco.json'],
      needs=['fetch_patco'], note='the two tracks, the stops and portals along them, the bridge-crossing trips and the resolved calendar: scheduled PATCO trains over the Ben Franklin Bridge'),
    S('bake_i95', 'bake_i95.py', inputs=['osm_city_raw.json'], outputs=['i95.json'], needs=['fetch_city'],
      note="I-95's motorway carriageways stitched into chains in the direction of travel: the page lines them with highway lamps, in the city and past it (Round 148)"),
    S('bake_delaware', 'bake_delaware.py', inputs=[], outputs=['delaware.json'], fetch=True,
      note='the tidal Delaware\'s outline from OSM coastline ways: the shoreline past the DEM grids and the river sheets past the ground box'),
    S('bake_towers', 'bake_towers.py',
      inputs=['wide_landmarks_research.json', 'wide_names.json', 'scene_wide.json', 'parts_wide.json'], outputs=['towers.json'],
      needs=['process_wide'], note='the researched Center City towers as facade archetype, crown and tint, matched to the wide footprints'),
    S('fetch_mapillary', 'fetch_mapillary.py',
      outputs=['lidar_cache/mly_images.json'], fetch=True, note='needs MAPILLARY_TOKEN; street-level thumbnails for the wall colours'),
    S('bake_wall_colors', 'bake_wall_colors.py',
      inputs=['lidar_cache/mly_images.json', 'scene_wide.json', 'scene_south.json'], outputs=['wall_palette.json', 'wall_colors.json'],
      needs=['fetch_mapillary', 'process_wide', 'process_south'], note='block-face wall colours from the thumbnails; pack_wide packs a real pass into wide_walls.b64 and ignores a dry run'),
    ...
    S('bake_traffic', 'bake_traffic.py',
      inputs=['osm_wide_raw.json', 'osm_south_raw.json', 'osm_city_raw.json', 'city_streets_raw.json?', 'lidar_cache/traffic_raw/rmstraffic.geojson'],
      outputs=['traffic.b64'], needs=['fetch_wide', 'fetch_south', 'fetch_city', 'fetch_traffic']),
    ...
    S('fetch_markets', 'fetch_markets.py',
      outputs=['lidar_cache/markets_raw/farmers_markets.geojson'], fetch=True,
      note="the City's Farmers_Markets points with hours, season and payments (ArcGIS); cached until deleted"),
    S('bake_markets', 'bake_markets.py',
      inputs=['lidar_cache/markets_raw/farmers_markets.geojson'],
      outputs=['markets.json'], needs=['fetch_markets'],
      note='the markets in the scene frame with their hours parsed for the clock (Round 76)'),
    S('fetch_civic', 'fetch_civic.py',
      outputs=['lidar_cache/civic_raw/libraries.json', 'lidar_cache/civic_raw/ppr_sites.json'], fetch=True,
      note="the Free Library's branches and Parks and Recreation's centers (City ArcGIS); cached until deleted"),
    S('bake_civic', 'bake_civic.py',
      inputs=['lidar_cache/civic_raw/libraries.json', 'lidar_cache/civic_raw/ppr_sites.json', 'scene.json?', 'wide.b64?', 'city.b64?'],
      outputs=['civic.json'], needs=['fetch_civic', 'process_core', 'pack_wide', 'pack_city'],
      note='the branches and centers in the scene frame, points stepped out of their own footprints (Round 134)'),
    S('fetch_septa_stops', 'fetch_septa_stops.py',
      outputs=['lidar_cache/septa_raw/gtfs_public.zip', 'lidar_cache/septa_raw/station_id_name.csv'], fetch=True,
      note="SEPTA's GTFS release (github.com/septadev/GTFS) and the Arrivals station list; cached until deleted"),
    S('bake_septa_stops', 'bake_septa_stops.py',
      inputs=['lidar_cache/septa_raw/gtfs_public.zip', 'lidar_cache/septa_raw/station_id_name.csv', 'city_limit.json', 'scene.json?', 'wide.b64?', 'city.b64?'],
      outputs=['septa_stops.json'], needs=['fetch_septa_stops', 'fetch_boundary', 'process_core', 'pack_wide', 'pack_city'],
      note='bus and trolley stops and Regional Rail stations inside the flight limit, in the scene frame (Round 133)'),
    S('fetch_markers', 'fetch_markers.py',
      outputs=['lidar_cache/markers_raw/phmc.json', 'lidar_cache/markers_raw/percent_for_art.geojson'], fetch=True,
      note='PHMC historical markers in Philadelphia (data.pa.gov) + the City Percent for Art works (ArcGIS); cached until deleted'),
    S('bake_markers', 'bake_markers.py',
      inputs=['lidar_cache/markers_raw/phmc.json', 'lidar_cache/markers_raw/percent_for_art.geojson', 'scene.json?', 'wide.b64?', 'city.b64?'],
      outputs=['markers.json'], needs=['fetch_markers', 'process_core', 'pack_wide', 'pack_city'],
      note='the markers and the artworks in the scene frame, texts cleaned, points inside a drawn footprint stepped out past its wall (Round 77)'),
    S('fetch_lights', 'fetch_lights.py',
      outputs=['lidar_cache/lights_raw/boma.json'], fetch=True,
      note="BOMA Philadelphia's Building Illumination Calendar (Squarespace JSON, every page); cached until deleted"),
    S('bake_lights', 'bake_lights.py',
      inputs=['lidar_cache/lights_raw/boma.json'],
      outputs=['lights.json'], needs=['fetch_lights'],
      note='the illumination calendar parsed to dated colour rows for the skyline lights (Round 81)'),
    S('fetch_landmarks', 'fetch_landmarks.py',
      outputs=['lidar_cache/landmarks_raw/poly.geojson', 'lidar_cache/landmarks_raw/points.geojson'], fetch=True,
      note='the City basemap Landmark_Poly + Landmark_Points (ArcGIS pages) for the search index; cached until deleted'),
    S('bake_landmarks', 'bake_landmarks.py',
      inputs=['lidar_cache/landmarks_raw/poly.geojson', 'lidar_cache/landmarks_raw/points.geojson'],
      outputs=['landmarks.json'], needs=['fetch_landmarks'],
      note='named places classed by subtype, sites grouped by parent, area-weighted centroids (Round 78)'),
    ...
    S('build', 'build.py',
      inputs=['template.html', 'style.css', 'three.min.js', 'app.js', 'scene.json', 'meta.json?', 'about_body.html?',
              ...
              'poles.b64?', 'paved.b64?', 'outskirts.b64?', 'city_limit.json?', 'parking_south.json?', 'storefronts.b64?', 'markets.json?', 'markers.json?', 'lights.json?', 'landmarks.json?', 'rail_amtrak.json?', 'patco.json?', 'i95.json?', 'septa_stops.json?', 'civic.json?', 'brand/dist/favicon.svg', 'brand/dist/favicon-32.png', 'brand/dist/apple-touch-icon.png'],
      outputs=['society-hill-towers.html'],
      needs=['process_core', 'pack_wide', 'pack_city', 'pack_outskirts', 'fetch_boundary', 'fetch_parking', 'bake_storefronts', 'bake_towers', 'bake_schuylkill', 'bake_delaware', 'bake_rail', 'bake_patco', 'bake_i95', 'patch_scenes_facade', 'bake_street_labels', 'bake_street_sdf',
             'pack_trees', 'bake_places', 'bake_markets', 'bake_markers', 'bake_lights', 'bake_landmarks', 'bake_septa_stops', 'bake_civic', 'bake_overpasses', 'bake_traffic', 'fetch_dem_nw', 'fetch_nw_parks',
             'fetch_nw_water', 'pack_poles', 'pack_paved', 'make_brand', 'fetch_wide', 'fetch_south', 'fetch_city'],
