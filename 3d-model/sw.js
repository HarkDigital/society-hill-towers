...
// its URL with no-store. A new Request built on a navigate request throws in Chrome, so the URL
// it is. The request itself is the fallback: offline (the page is no use offline anyway) and on a
...
self.addEventListener('fetch', (e) => {
  ...
  e.respondWith(fetch(e.request.url, { cache: 'no-store', credentials: 'same-origin' })
