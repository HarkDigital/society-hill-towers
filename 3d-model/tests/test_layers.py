...
KEYS = ['septa', 'indego', 'flights', 'ships', 'traffic', 'lights', 'streets', 'labels', 'places', 'concerts', 'amtrak', 'closures', 'markers', 'art']
...
class Layers(unittest.TestCase):
    ...
    def test_mask(self):
        ...
        self.assertIn('const LAYER_MASK_V2 = 1024, LAYER_MASK_V3 = 4096, LAYER_MASK_V4 = 16384;', self.src)
        self.assertIn('let m = LAYER_MASK_V4;', self.src, 'a new link carries the fourteen-bit marker alone')
        self.assertIn('(m & LAYER_MASK_V4) ? LAYER_KEYS.length : (m & LAYER_MASK_V3) ? 12 : (m & LAYER_MASK_V2) ? 10 : 9', self.src, 'older links keep their width')
        self.assertIn('out.l = v[0] & 32767;', self.src, 'the hash keeps fifteen bits')
        for k in KEYS:
            self.assertIn("'%s' in f" % k, self.src, 'setLayerFlags does not read ' + k)
            self.assertIn('%s: ' % k, self.src)
        self.assertIn('markers: toggleMarkers, art: toggleArt', self.src, 'Reset Layers must know every key')
    ...
    def test_rows_and_keys(self):
        ...
        for key, fn in (('j', 'toggleMarkers'), ('o', 'toggleArt'), ('u', 'toggleClosures'), ('k', 'toggleAmtrak')):
