...
(function () {
  ...
  const camera = new THREE.PerspectiveCamera(58, window.innerWidth / window.innerHeight, 0.75, 9000);
  ...
  // classical column row: n cylinders from (x0,z0) to (x1,z1)
  function columnRow(x0, z0, x1, z1, n, h, r, yBase) {
    const geoms = [];
    for (let i = 0; i < n; i++) {
      const t = n === 1 ? 0.5 : i / (n - 1);
      const g = new THREE.CylinderGeometry(r * 0.88, r, h, 10);
      g.translate(x0 + (x1 - x0) * t, (yBase || 0) + h / 2, z0 + (z1 - z0) * t);
      geoms.push(g);
    }
    return geoms;
  }
  ...
  step('Laying out the ground', () => {
    ...
    for (let pi = 0; pi < POOLS.length; pi++) {
      ...
      if (pi === 0) { // paved pool deck around the towers' pool
        ...
        const m = 5;
        const deck = [
          [ob.cx - a.ax * (a.hl + m) - a.px * (a.hs + m), ob.cz - a.az * (a.hl + m) - a.pz * (a.hs + m)],
          [ob.cx + a.ax * (a.hl + m) - a.px * (a.hs + m), ob.cz + a.az * (a.hl + m) - a.pz * (a.hs + m)],
          [ob.cx + a.ax * (a.hl + m) + a.px * (a.hs + m), ob.cz + a.az * (a.hl + m) + a.pz * (a.hs + m)],
          [ob.cx - a.ax * (a.hl + m) + a.px * (a.hs + m), ob.cz - a.az * (a.hl + m) + a.pz * (a.hs + m)],
        ];
        deckParts.push({ geom: flatPoly(deck, null, LAYER.deck), color: new THREE.Color(0xb3ab9c) });
        registerPoly(deck);
      }
    }
    ...
    cityMat.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace('#include <color_fragment>', [
          ...
          '      float wm = rectM(m, vec2(0.0, 1.6), vec2(pitch * 0.66, 1.75), aa) * step(0.9, v) * okRow;',
  ...
  step('Restoring the landmarks', () => {
    ...
    function classic(name, o) {
      ...
      if (o.roof === 'hip') {
        ...
        const sp = simplifyRing(b.poly, 0.6);
        if (sp.length === 4) {
          ...
          const g = gableGeom(ob, o.eave, o.ridge, 0.38, 0);
          ad(g.slopes, o.roofCol);
          if (o.gableEndsTrim) ad(g.ends, o.trim); else aw(g.ends, o.wall, st);
          for (const s of [-1, 1]) {
            ad(box(g.hl * 2 + 0.05, 0.4, 0.5,
              ob.cx + g.px * s * (g.hs - 0.22), o.eave - 0.1, ob.cz + g.pz * s * (g.hs - 0.22),
              ryAlign(g.ax, g.az)), o.trim);
          }
        }
      }
      if (o.chimneys) {
    ...
    // The Ryland: two offset pale-glass bars with a white metal grid
    {
      ...
      if (b) {
        ...
        for (const bar of bars) {
          ...
          glassParts2.push({ geom: box(L, bar.h, bar.d, bx, bar.h / 2, bz, ry), color: new THREE.Color(0xbccfdb) });
          // white metal grid: verticals every ~3 m, heavier band every 3 floors
          ...
          for (let i = 0; i <= nV; i++) {
            ...
            for (const s of [-1, 1]) {
              ad(box(0.16, bar.h, 0.14,
                ...
                bz + a.az * u + a.pz * s * (bar.d / 2), ry), '#e8eaeb');
            }
          }
        ...
        glassParts2.push({
          ...
          color: new THREE.Color(0xdde6ea),
        });
    ...
    }
    {
      ...
      if (r) { // Doric octastyle porticos on both short fronts
        const m0 = mark();
  ...
  step('Casting the concrete grid', () => {
    ...
    for (let ti = 0; ti < towers.length; ti++) {
      ...
      for (const s of sides) {
        ...
        const bays = Math.max(4, Math.round(inner / TOWER.bayPitch)); // -> 17-18 / 11-12
        ...
        for (let ci = 0; ci <= nCols; ci++) {
          ...
          const g = s.axis === 'x'
            ? box(0.55, gridBot, 0.55, u, gridBot / 2, s.off)
            : box(0.55, gridBot, 0.55, s.off, gridBot / 2, u);
          add(concreteParts, g, cCon);
        }
      }
      ...
      // lobby glass, deeply recessed behind the colonnade
      add(glassParts,
        box(W - TOWER.lobbyInset * 2, gridBot - 0.25, Dp - TOWER.lobbyInset * 2, 0, (gridBot - 0.25) / 2, 0),
  ...
  step('Raising the outer districts', async () => {
    ...
    for (let i = 0; i < nb; i++) {
      ...
      c.set(pool[Math.floor(hsh * pool.length) % pool.length]).multiplyScalar(0.9 + hash01(i * 11.3) * 0.2);
  ...
  step('Planting the street trees', () => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      cCan.setHSL(0.26 + hash01(i * 4.9) * 0.05, 0.4 + hash01(i * 6.1) * 0.14, 0.2 + hash01(i * 8.3) * 0.07);
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let k = 0; k < 2; k++) {
        ...
        cCan.setHSL(0.26 + hash01(i * 4.9) * 0.045, 0.32 + hash01(i * 6.1) * 0.12, 0.29 + hash01(i * 8.3) * 0.08 + 0.025 * k);
