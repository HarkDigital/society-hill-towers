...
| `bake_schuylkill.py` → `schuylkill.json`, `lidar_cache/schuylkill_ways.json` | The Schuylkill's real course: the OSM `waterway=river` ways named Schuylkill over the city (Overpass, cached), every fragment simplified to 3 m for the terrain carve, and their 65 m buffer united and clipped to the dam-to-East-Falls reach as the water sheet with islands as holes. The hand polyline in app.js is the fallback. |
| `schuylkill.json` | Inlined as `SCHUYLKILL_DATA`: `schuylkillCut` and `riverCorridor` follow its `lines`; "Raising the rest of Philadelphia" draws its `water` flat at the river level. |
