...
- Owner decisions that stand until Mike says otherwise: landmark labels OFF by default (the
  ...
  their pins depth-tested), neighborhood names and the search pin are not; every flat pin carries its ANCHOR's depth on
  all four corners and writes it (Round 126, Mike: farther pins painted over nearer ones and facades sliced the quads;
  `PIN_ANCHOR_GLSL` in `pinSceneDepth`, `alphaTest` 0.5, only a wall in front of the tip hides a pin, the pins sort
  among themselves whatever the draw order; `tests/test_pins.py` guards it); the Marriott Old City's porte-cochere
