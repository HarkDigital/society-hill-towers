...
farmers' markets on the clock, solar clock + weather (and the air quality), build & loop; the post pipeline sits with the renderer near the top. Build steps are `step('Name', fn)` calls run in order by
