...
- Owner decisions that stand until Mike says otherwise: landmark labels OFF by default (the
  ...
  on the day of a Philadelphia game, home or away (Round 90, Mike: have the sporting events act like
  the concerts for the day of). The bubble rises at 9 am Philadelphia time on the day of the game
  (`scoreDayStart`, the same instant `ops/concerts_bake.py` bakes for a show) and stands until an
  hour past the final; before the first pitch it reads "PHI vs NYM" or "PHI at NYM" with the start
  time and wears a still bronze dot (`.lbl.score.pre`), and the banner carries Home or Away
  capitalised. Judged against REAL time, never the pinned model clock, like the concerts. A
  postponed or canceled game gets no bubble, though ESPN reports it in the same `pre` state. Ticketmaster's listings (Round 56: `ops/concerts_bake.py` bakes them
