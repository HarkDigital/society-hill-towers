...
- Owner decisions that stand until Mike says otherwise: landmark labels OFF by default (the
  ...
  values); weather alerts show in `#notice` and on the time panel (Round 131); fly is the only mode
  ...
  among themselves; a small depth image of the city (`pinOccCapture`, every 5th frame, 10th on touch) decides per tip
  whether a building hides it (`aPinVis`, `pinOccVisible`), and the concert placards and score bubbles ask the same
